...
def print_info(product, customName, pcf):
	...
	print(
		...
		% (
			...
			", ".join("{package}({condition}{version})".format(**dep) for dep in pcf.getPackageDependencies()),
		)
	)
...
def parse_args(args: List[str] | None = None):
	...
	parser.add_argument(
		"--archive-format", "-F", dest="format", default="cpio", choices=["cpio", "tar"], help="Archive format to use. Default: cpio"
	)
	parser.add_argument("--no-pigz", dest="disablePigz", default=False, action="store_true", help="Disable the usage of pigz")
	parser.add_argument(
		"--no-set-rights", dest="no_set_rights", default=False, action="store_true", help="Disable the setting of rights while building"
	)
	...
	hashSumGroup.add_argument(
		"--md5", "-m", dest="createMd5SumFile", default=True, action="store_true", help="Create file with md5 checksum."
	)
	...
	zsyncGroup.add_argument("--zsync", "-z", dest="createZsyncFile", default=True, action="store_true", help="Create zsync file.")
	...
	vgroup.add_argument(
		"--product-version", default="", dest="newProductVersion", metavar="productversion", help="Set new product version for package"
	)
						...
						try:
							while True:
								...
								if ch in ("o", "O"):
									...
									break
								if ch in ("c", "C"):
									raise Exception(_("Aborted"))  # pylint: disable=broad-exception-raised
								if ch in ("n", "N"):
									newVersion = True
									break
						finally:
		...
		if pid:
			for line in execute("ps -A"):
				...
				if pid == line.split()[0].strip():
					...
					# process is running
					raise RuntimeError(f"Product '{packageControlFile.getProduct().id}' is currently locked by process {pName} ({pid}).")
