...
def opsisetup_main():  # pylint: disable=too-many-branches.too-many-statements
	...
	for (opt, arg) in opts:
		if opt == "--log-file":
			...
			logger.warning("file-to-mysql is deprecated. The task is performed automatically at the start of opsiconfd.")
			return
			...
			logger.warning("auto-configure-samba is deprecated. The task is performed automatically at the start of opsiconfd.")
			return
		elif opt == "--auto-configure-dhcpd":
			logger.warning("auto-configure-dhcp is deprecated. The task is performed automatically at the start of opsiconfd.")
			return
		elif opt == "--patch-sudoers-file":
			logger.warning("patch-sudoers-file is deprecated. The task is performed automatically at the start of opsiconfd.")
			return
