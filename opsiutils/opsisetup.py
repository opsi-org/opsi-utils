				...
				try:
					with JSONRPCBackend(address=configserver.id, username=depot.id, password=depot.opsiHostKey) as jsonrpcBackend:
						password = blowfishDecrypt(
							depot.opsiHostKey,
							jsonrpcBackend.user_getCredentials(username=CLIENT_USER, hostId=depot.id)[  # pylint: disable=no-member
								"password"
							],
						)
...
def configureMySQLBackend(unattendedConfiguration=None):
	...
	def errorFunction(*args):
		...
		ui.showError(text=message, width=70, height=6, title="Problem configuring MySQL backend")
	...
	if unattendedConfiguration is not None:
		...
		for key in ("dbAdminUser", "dbAdminPass"):
			if key not in unattendedConfiguration:
				raise Exception(errorTemplate.format(key=key))  # pylint: disable=broad-exception-raised
	...
	try:
		while True:
			...
			if values is None:
				raise Exception("Canceled")  # pylint: disable=broad-exception-raised
			...
			messageBox = ui.createMessageBox(width=70, height=20, title="MySQL config", text="")
		...
		time.sleep(2)
		ui.showMessage(width=70, height=4, title="Success", text="MySQL Backend configuration done")
	finally:
...
def registerDepot(unattendedConfiguration=None):
	...
	if not depots:
		raise Exception("Failed to create depot")  # pylint: disable=broad-exception-raised
	depot = depots[0]
	...
	try:
		...
		raise Exception(f"Failed to connect to config server as user '{config['username']}': {err}") from err
	...
	with codecs.open(dispatchConfigFile, "r", "utf-8") as originalDispatchConfig:
		lines = [line for line in originalDispatchConfig if line.strip().startswith((";", "#"))]
...
def _getJSONRPCBackendFromConfig(config):
	...
	"""
	connectionConfig = {"address": config["address"], "username": config["username"], "password": config["password"]}
	...
	if not jsonrpcBackend.accessControl_userIsAdmin():  # pylint: disable=no-member
		raise Exception(f"User {connectionConfig['username']} is not an admin user")  # pylint: disable=broad-exception-raised

	logger.notice(
		"Successfully connected to config server '%s' as user '%s'", connectionConfig.get("address"), connectionConfig.get("username")
	)
...
def _getBackendConfigViaGUI(config):  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
	...
	try:
		...
		while True:
			...
			try:
				jsonrpcBackend = _getJSONRPCBackendFromConfig({"address": config["address"], "username": adminUser, "password": adminPass})
			except Exception as err:  # pylint: disable=broad-except
				messageBox.hide()
				err_text = f"Failed to connect to config server '{config['address']}' as user '{adminUser}': {err}"
				logger.error(err_text)
				ui.showError(title="Failed to connect", width=70, height=6, seconds=0, text=err_text)
				continue

			messageBox.addText(f"Successfully connected to config server '{config['address']}' as user '{adminUser}'\n")
			break
...
def opsisetup_main():  # pylint: disable=too-many-branches.too-many-statements
	...
	if os.geteuid() != 0:
		raise Exception("This script must be startet as root")  # pylint: disable=broad-exception-raised
	...
	if len(args) > 0:
		...
		if task == "set-rights" and len(args) == 1:
			...
			usage()
			raise Exception("Too many arguments")  # pylint: disable=broad-exception-raised

	if noBackup and task != "file-to-mysql":
		raise Exception("--no-backup only valid with --file-to-mysql")  # pylint: disable=broad-exception-raised
	if noRestart and task != "file-to-mysql":
		raise Exception("--no-restart only valid with --file-to-mysql")  # pylint: disable=broad-exception-raised
