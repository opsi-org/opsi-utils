...
def main() -> None:
	try:
		...
		if args.clients:
			opsi_cli_call.extend(["--clients", args.clients])
		if args.client_groups:
			opsi_cli_call.extend(["--client-groups", args.client_groups])
		if args.exclude_client_groups:
			opsi_cli_call.extend(["--exclude-client-groups", args.exclude_client_groups])
		opsi_cli_call.extend(["set-action-request", "--where-outdated"])
		if args.add_failed:
		...
		if args.include_products:
			opsi_cli_call.extend(["--products", args.include_products])
		if args.include_product_groups:
			opsi_cli_call.extend(["--product-groups", args.include_product_groups])
		if args.exclude_products:
			opsi_cli_call.extend(["--exclude-products", args.exclude_products])
		if args.exclude_product_groups:
			opsi_cli_call.extend(["--exclude-product-groups", args.exclude_product_groups])
